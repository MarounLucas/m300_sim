...
import yaml
import os
import requests # <--- IMPORTANTE: Adicionado requests para API
import numpy as np
...
# --- CONFIGURAÇÃO MATHTEXT PARA MATPLOTLIB ---
...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QFormLayout, 
                               ...
                               QRadioButton, QComboBox, QMessageBox, QStyle,
...
# =========================================================================
# CLASSES AUXILIARES (WORKER, CUSTOM MSGBOX E DIALOG DE AJUDA)
# =========================================================================
class MissionHelpDialog(QDialog):
    def __init__(self, parent=None):
        ...
        <style>
            ...
            b { color: #ffffff; }
            code { background-color: #444; padding: 2px 4px; border-radius: 3px; color: #ffcc00;}
        </style>
        </head>
        <body>
        <h2>Mission Configuration Guide</h2>
        <p>This section allows you to define the UAV's flight path, trajectory parameters, and environmental conditions for the integration algorithms.</p>
        
        <h3>1. Waypoints Configuration</h3>
        <ul>
            <li><b>Coordinate System:</b> Choose between Cartesian (X, Y, Z in meters) or Geodesic (Latitude, Longitude, Altitude).</li>
            <li><b>Adding Points:</b> Manually input coordinates or load a batch from a <code>.csv</code> or <code>.json</code> file.</li>
            <li><b>Table Tools:</b> Select rows to Edit, Duplicate, Reorder, or Remove waypoints dynamically.</li>
        </ul>

        <h3>2. Trajectory Planning</h3>
        <ul>
            <li><b>Generation Type:</b> Mathematical method to interpolate waypoints (e.g., 5th Degree Polynomial for smooth, jerk-free motion profiles).</li>
            <li><b>Max Speed (XY) [m/s]:</b> Maximum horizontal speed allowed during the entire mission.</li>
            <li><b>Max Speed (Z) [m/s]:</b> Maximum vertical speed allowed for ascents and descents.</li>
        ...
        <ul>
            <li><b>Wind Type:</b> Simulation wind disturbance model (None, Constant, Dryden Gust, Dryden Low-Pass, Sinusoid).</li>
            <li><b>Base Magnitude [m/s]:</b> Base persistent wind speed.</li>
            <li><b>Heading / Elevation [&deg;]:</b> Wind direction vector angles relative to the global frame.</li>
            <li><b>Max Gust [m/s]:</b> Maximum magnitude added dynamically by gusts (if an active model is selected).</li>
            <li><b>OpenWeather Data:</b> Connects to real-time meteorological servers based on global coordinates to fetch live wind speeds, heading and gusts!</li>
        </ul>
        </body>
        </html>
        """
...
class TabMission(QWidget):
    ...
    def _setup_dark_theme(self):
        ...
        self.setStyleSheet("""
            QGroupBox { border: 1px solid #555555; border-radius: 4px; margin-top: 1ex; padding-top: 10px; }
            QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top left; padding: 0 3px; color: #dddddd; }
        """)
    ...
    def _build_ui(self):
        ...
        main_tab_layout.addLayout(global_toolbar_layout)
        ...
        # 1. Waypoints Configuration
        self.waypoints_group = QGroupBox("1. Waypoints Configuration")
        self.waypoints_group.setFixedSize(850, 400) 
        ...
        self.label_x = QLabel("X [m]:"); self.label_x.setFixedWidth(90)
        self.label_y = QLabel("Y [m]:"); self.label_y.setFixedWidth(90)
        self.label_z = QLabel("Z [m]:"); self.label_z.setFixedWidth(90)
        
        self.spin_x = QDoubleSpinBox(); self.spin_x.setRange(-9999999.99, 9999999.99); self.spin_x.setDecimals(6)
        self.spin_y = QDoubleSpinBox(); self.spin_y.setRange(-9999999.99, 9999999.99); self.spin_y.setDecimals(6)
        self.spin_z = QDoubleSpinBox(); self.spin_z.setRange(-9999.99, 9999.99); self.spin_z.setDecimals(2)
        ...
        self.btn_edit = QPushButton(); self.btn_edit.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogDetailedView))
        self.btn_dup = QPushButton(); self.btn_dup.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_TitleBarNormalButton))
        self.btn_up = QPushButton(); self.btn_up.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_ArrowUp))
        self.btn_down = QPushButton(); self.btn_down.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_ArrowDown))
        self.btn_rem = QPushButton(); self.btn_rem.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_TrashIcon))
        ...
        toolbar_layout.addWidget(self.btn_edit); toolbar_layout.addWidget(self.btn_dup)
        toolbar_layout.addWidget(self.btn_up); toolbar_layout.addWidget(self.btn_down)
        ...
        self.waypoints_group.setLayout(wp_internal_layout)
        left_master_container.addWidget(self.waypoints_group)

        # 2. Trajectory Planning 
        self.trajectory_group = QGroupBox("2. Trajectory Planning")
        self.trajectory_group.setFixedSize(850, 150) 
        ...
        self.spin_speed_xy.setRange(0.1, 30.0); self.spin_speed_xy.setValue(10.0); self.spin_speed_xy.setSuffix(" m/s")
        self.spin_speed_z = QDoubleSpinBox()
        self.spin_speed_z.setRange(0.1, 15.0); self.spin_speed_z.setValue(5.0); self.spin_speed_z.setSuffix(" m/s")
        ...
        self.combo_yaw_mode.addItems(["Free (Initial Fixed)", "Forward (Tangent)", "Target (Fixed)"])
        ...
        self.spin_yaw_x = QDoubleSpinBox(); self.spin_yaw_x.setDecimals(1); self.spin_yaw_x.setPrefix("X: ")
        self.spin_yaw_y = QDoubleSpinBox(); self.spin_yaw_y.setDecimals(1); self.spin_yaw_y.setPrefix("Y: ")
        self.spin_yaw_z = QDoubleSpinBox(); self.spin_yaw_z.setDecimals(1); self.spin_yaw_z.setPrefix("Z: ")
        yaw_target_layout.addWidget(self.spin_yaw_x); yaw_target_layout.addWidget(self.spin_yaw_y); yaw_target_layout.addWidget(self.spin_yaw_z)
        ...
        left_master_container.addWidget(self.trajectory_group)
        self.toggle_yaw_target()

        # 3. Environment & Location
        self.env_group = QGroupBox("3. Environment & Location Parameters")
        self.env_group.setFixedSize(850, 250) 
        ...
        self.combo_wind_type.addItems(["None", "Constant", "Dryden Gust", "Dryden Low-Pass", "Sinusoid"])
        self.spin_wind_mag = QDoubleSpinBox(); self.spin_wind_mag.setRange(0, 100); self.spin_wind_mag.setSuffix(" m/s")
        
        lbl_heading = QLabel("Heading [&deg;]:"); lbl_heading.setTextFormat(Qt.TextFormat.RichText)
        lbl_elev = QLabel("Elevation [&deg;]:"); lbl_elev.setTextFormat(Qt.TextFormat.RichText)

        self.spin_wind_head = QDoubleSpinBox(); self.spin_wind_head.setRange(-360, 360); self.spin_wind_head.setSuffix(" °")
        self.spin_wind_elev = QDoubleSpinBox(); self.spin_wind_elev.setRange(-90, 90); self.spin_wind_elev.setSuffix(" °")
        self.spin_wind_gust = QDoubleSpinBox(); self.spin_wind_gust.setRange(0, 100); self.spin_wind_gust.setSuffix(" m/s")
        ...
        wind_layout.addLayout(wind_form_left); wind_layout.addLayout(wind_form_right)
        env_layout.addLayout(wind_layout)
        
        line = QWidget(); line.setFixedHeight(1); line.setStyleSheet("background-color: #555555;")
        env_layout.addWidget(line)
        
        loc_layout = QHBoxLayout()
        loc_form = QFormLayout()
        self.spin_loc_lat = QDoubleSpinBox(); self.spin_loc_lat.setRange(-90, 90); self.spin_loc_lat.setDecimals(6)
        self.spin_loc_lat.setValue(-22.739000) # Injectando padrao para Piracicaba!
        
        self.spin_loc_lon = QDoubleSpinBox(); self.spin_loc_lon.setRange(-180, 180); self.spin_loc_lon.setDecimals(6)
        self.spin_loc_lon.setValue(-47.646000) # Injectando padrao para Piracicaba!
        
        loc_form.addRow("Latitude:", self.spin_loc_lat); loc_form.addRow("Longitude:", self.spin_loc_lon)
        
        btn_fetch_weather = QPushButton(" OpenWeather Data")
        btn_fetch_weather.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_ComputerIcon))
        btn_fetch_weather.clicked.connect(self.fetch_weather_data)
        
        loc_layout.addLayout(loc_form); loc_layout.addWidget(btn_fetch_weather); loc_layout.addStretch()
        env_layout.addLayout(loc_layout)
        self.env_group.setLayout(env_layout)
        left_master_container.addWidget(self.env_group)
        left_master_container.addStretch() 

        # --- RIGHT SIDE: VISUALIZER ---
        self.visualizer_group = QGroupBox("Trajectory Visualizer")
        vis_layout = QVBoxLayout()
        
        vis_controls = QHBoxLayout()
        self.view_combo = QComboBox()
        self.view_combo.addItems(["3D Perspective", "Top View (XY)", "Side View (YZ)", "Front View (XZ)"])
        ...
        vis_controls.addWidget(QLabel("Camera:")); vis_controls.addWidget(self.view_combo)
        ...
        self.visualizer_group.setLayout(vis_layout)

        split_layout.addWidget(self.left_master_container_widget) 
        split_layout.addWidget(self.visualizer_group, 1) 
    ...
    def update_plot(self):
        ...
        if "3D" in view_mode:
            ...
            try: self.ax.set_box_aspect(None) 
            except Exception: pass 

            if x_vals:
                self.ax.plot(x_vals, y_vals, z_vals, linestyle='-', color='#00d4ff', linewidth=1.5)
                self.ax.scatter(x_vals, y_vals, z_vals, color='white', s=25)
                self.ax.plot([x_vals[0]], [y_vals[0]], [z_vals[0]], marker='s', color='#4caf50', markersize=8)
                if len(x_vals) > 1:
                    self.ax.plot([x_vals[-1]], [y_vals[-1]], [z_vals[-1]], marker='X', color='#f44336', markersize=8)
            ...
            if x_vals:
                ...
                self.ax.plot(h, v, linestyle='-', color='#00d4ff', marker='o', markerfacecolor='white')
                self.ax.plot([h[0]], [v[0]], marker='s', color='#4caf50', markersize=8)
                if len(h) > 1: self.ax.plot([h[-1]], [v[-1]], marker='X', color='#f44336', markersize=8)
                if "YZ" in view_mode or "XZ" in view_mode: self.ax.invert_yaxis()
    ...
    # =========================================================================
    # INTEGRAÇÃO DO MOTOR MATEMÁTICO E LÓGICAS GERAIS
    # =========================================================================
    def start_simulation(self):
        ...
        if not mission_data.get("waypoints"):
            msg = CustomMessageBox("Warning", "You need to define at least one Waypoint in the Mission tab.", msg_type="info", parent=self)
            msg.exec()
            return

        if aircraft_data.get("mass", 0.0) <= 0.01:
            msg = CustomMessageBox("Error", "Aircraft parameters are empty!", "Please go to the 'Aircraft Parameters' tab and load a valid drone model.", msg_type="error", parent=self)
            msg.exec()
            return
        ...
        try:
            ...
            msg = CustomMessageBox("Export Error", "Could not export parameters to ROS YAML.", str(e), msg_type="error", parent=self)
            msg.exec()
            return
    ...
    def reset_mission(self):
        msg = CustomMessageBox("New Mission", "Do you want to reset everything?", "Unsaved data will be lost.", msg_type="question", parent=self)
    ...
    def save_mission(self):
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Mission", str(MISSIONS_DIR), "Mission Files (*.json)")
        if not file_path: return
        if not file_path.endswith('.json'): file_path += '.json'
        mission_data = self._collect_mission_data() 
        try:
            with open(file_path, 'w', encoding='utf-8') as f: json.dump(mission_data, f, indent=4)
            self.export_to_ros_yaml() 
            msg = CustomMessageBox("Success", "Mission saved successfully!", f"File saved at:\n{file_path}", msg_type="success", parent=self)
            msg.exec()
        except Exception as e:
            msg = CustomMessageBox("Save Error", "Could not save the file.", str(e), msg_type="error", parent=self)
            msg.exec()
    ...
    def export_to_ros_yaml(self):
        ...
        if "Forward" in yaw_mode_ui: yaw_mode = "forward"
        elif "Target" in yaw_mode_ui: yaw_mode = "target"
        else: yaw_mode = "free"
        ...
        with open(yaml_path_src, 'w', encoding='utf-8') as f:
            yaml.dump(ros_data, f, default_flow_style=None, sort_keys=False)
    ...
    def load_mission(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Load Mission", str(MISSIONS_DIR), "Mission Files (*.json)")
        if not file_path: return
        try:
            ...
            if data.get("coordinate_system") == "cartesian": self.radio_cartesian.setChecked(True)
            else: self.radio_geodesic.setChecked(True)
            ...
            self.table_waypoints.setRowCount(0) 
            for wp in data.get("waypoints", []): self._add_row_to_table(wp.get("x", 0.0), wp.get("y", 0.0), wp.get("z", 0.0))
            ...
            msg = CustomMessageBox("Success", "Mission loaded!", f"Model '{filename}' loaded successfully.", msg_type="success", parent=self)
            msg.exec()
            
        except Exception as e:
            self.table_waypoints.blockSignals(False) 
            msg = CustomMessageBox("Read Error", "The selected file is not a valid Mission.", str(e), msg_type="error", parent=self)
            msg.exec()

    def toggle_gust_input(self):
        wind_type = self.combo_wind_type.currentText()
        if "None" in wind_type or "Constant" in wind_type:
            self.spin_wind_gust.setEnabled(False); self.spin_wind_gust.setValue(0.0)
        else:
            self.spin_wind_gust.setEnabled(True)

    # -------------------------------------------------------------------------
    # NOVO: API DO OPENWEATHER INTEGRADA À UI
    # -------------------------------------------------------------------------
    def fetch_weather_data(self):
        ...
        if lat == 0.0 and lon == 0.0:
            msg = CustomMessageBox("Warning", "Please enter valid Latitude and Longitude.", msg_type="info", parent=self)
            msg.exec()
            return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                detail = f"Weather: {sky.title()}\nTemperature: {temp}°C\nWind Speed: {wind_speed} m/s\nWind Gust: {wind_gust} m/s"
                msg = CustomMessageBox("OpenWeather Success", "Live weather data fetched and applied!", detail, msg_type="success", parent=self)
                msg.exec()
            else:
                msg = CustomMessageBox("API Error", f"Server returned status code: {response.status_code}", msg_type="error", parent=self)
                msg.exec()
        except Exception as e:
            self.unsetCursor()
            msg = CustomMessageBox("Connection Error", "Failed to connect to OpenWeather API.", str(e), msg_type="error", parent=self)
            msg.exec()
            
    def update_coordinate_labels(self):
        if self.radio_cartesian.isChecked():
            self.label_x.setText("X [m]:"); self.label_y.setText("Y [m]:"); self.label_z.setText("Z [m]:")
            self.table_waypoints.setHorizontalHeaderLabels(["X [m]", "Y [m]", "Z [m]"])
        else:
            self.label_x.setText("Latitude:"); self.label_y.setText("Longitude:"); self.label_z.setText("Altitude [m]:")
            self.table_waypoints.setHorizontalHeaderLabels(["Latitude", "Longitude", "Altitude [m]"])

    def toggle_toolbar(self):
        selected_rows = set(item.row() for item in self.table_waypoints.selectedItems())
        count = len(selected_rows)
        self.btn_edit.setEnabled(count == 1); self.btn_up.setEnabled(count == 1); self.btn_down.setEnabled(count == 1)
        self.btn_dup.setEnabled(count >= 1); self.btn_rem.setEnabled(count >= 1)

    def handle_item_click(self, item):
        row = item.row()
        if self._last_selected_row == row: self.table_waypoints.clearSelection(); self._last_selected_row = -1
        else: self._last_selected_row = row

    def _add_row_to_table(self, x, y, z):
    ...
    def add_manual_waypoint(self):
        self._add_row_to_table(self.spin_x.value(), self.spin_y.value(), self.spin_z.value())
        self.spin_x.setValue(0.0); self.spin_y.setValue(0.0); self.spin_z.setValue(0.0)

    def edit_waypoint(self):
        current_row = self.table_waypoints.currentRow()
        if current_row >= 0: self.table_waypoints.editItem(self.table_waypoints.item(current_row, 0))
    ...
    def remove_waypoint(self):
        ...
        for row in rows_to_remove: self.table_waypoints.removeRow(row)
    ...
    def duplicate_waypoint(self):
        ...
        for row in reversed(rows_to_duplicate):
            ...
            self.table_waypoints.setItem(insert_position, 0, QTableWidgetItem(f"{x:.{dec}f}"))
            self.table_waypoints.setItem(insert_position, 1, QTableWidgetItem(f"{y:.{dec}f}"))
            self.table_waypoints.setItem(insert_position, 2, QTableWidgetItem(f"{z:.2f}"))
        self.table_waypoints.blockSignals(False) 
        self.update_plot()
    ...
    def load_waypoints_from_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Waypoints file", base_dir, "Files (*.csv *.json)")
        if not file_path: return 
        ...
        try:
            if file_path.endswith('.csv'):
                ...
                with open(file_path, mode='r', encoding='utf-8') as f:
                    data = json.load(f)
                    if isinstance(data, dict) and "waypoints" in data: data = data["waypoints"]
                    for item in data:
            ...
            msg = CustomMessageBox("Read Error", "Could not read the file.", str(e), msg_type="error", parent=self)
            msg.exec()
        finally:
            self.table_waypoints.blockSignals(False) 
            self.update_plot() 
